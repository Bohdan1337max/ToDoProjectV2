...
function Todo({todo, setTodos, todos}) {
    ...
    const editButtonHandle = () => {
        if (isEdited) {
            ...
            setTodos((prev) =>
                prev.map((el) =>
                    el.id === todo.id ? {...el, name: updatedName} : el
                )
            );
        }
    ...
    return (
        <div className={`post ${isCompleted ? "completed" : ""}`}>
            {isEdited
            ...
            <div className={"edit-buttons"} role={"group"}>
                ...
                <div className={"complete-button"} onClick = {completeTask}>{isCompleted ? "Uncomplete" : "Complete"}</div>
